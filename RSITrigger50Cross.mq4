//+------------------------------------------------------------------+
//|                                            RSITrigger50Cross.mq4 |
//|                                              lefterisk@gmail.com |
//|                                              http://www.mql4.com |
//+------------------------------------------------------------------+
//| Daily RSI crossing strategy for EURUSD                           |
//| Long when crossing above 50, short when crossing below 50        |
//+------------------------------------------------------------------+
...
int USER_MAGIC_DAILY_LONG=905;                                       // Identifies this EA's long positions
int USER_MAGIC_DAILY_SHORT=906;                                      // Identifies this EA's short positions
...
void OnTick()
{
   ...
   if (
      IsOpen(USER_MAGIC_DAILY_SHORT) &&
      CloseRSI50CrossShort()
   ){
      actionLog("CloseRSI50CrossShort");
      int shorts[10], i=0;
...
void Log(string tag)
{
      ...
      Print(tag, ": OpenLong=", OpenRSI50CrossLong(), ", OpenShort=", OpenRSI50CrossShort());
}
...
//+------------------------------------------------------------------+
//| User function OpenRSI50CrossLong().                                 |
//| Returns true when long opening conditions are met according to   |
//| EA rules                                                         |
//+------------------------------------------------------------------+
bool OpenRSI50CrossLong()
{  
   return (
      //rsi_1 > 30.0 && rsi_2 < 30.0
      rsi_1 > 50.0 && rsi_2 < 50.0
   );
}

bool CloseRSI50CrossLong()
{
   return (
